        ...
        if (Auth::guard('members')->attempt(['email' => $request->email, 'password' => $request->password,'status'=>'1'], $request->get('remember'))) {
            ...
            if((Session::has('ProgramID'))&&(Session::has('CoachID'))){
                ...
                // $mp = member_programs::find($id);
                 $program = member_programs::firstOrCreate(
                    ['program_id' => $pid,
                    'coache_id'=>$cid],
                    ['member_id' =>$mid],
                 );
            ...
            return redirect( route('memberdashboard') )->with([ 'message' => sprintf('Great! You have Successfully loggedin And successfully participated in the program "%s"', $pid),
            'alert-type' => 'success'
            ]);
                }else{
