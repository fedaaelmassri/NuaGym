...
class EventsController extends Controller
{
    ...
    public function eventsformember(){
        ...
        $event = events_member::where('member_id', $mid)->get();
          return view('frontend.member.events')->with([
            'Events' => $event,
         ]);

       }
    public function subscripeEvent($eventid,$cost){
        ...
        if(!auth()->guard('members')->check()){
            ...
            $mid= Auth::guard('members')->user()->id;
            $report = subscriptions::where('payable_id', '=', $eventid)
            ->where('member_id', '=', $mid)
            ...
            $event = subscriptions::updateOrCreate(
                ['payable_id' => $eventid],
                ['member_id' =>$mid],
                ['payable_type' =>'event'],
                ['cost' =>$cost],
