                         ...
                         <input type="text" id="eventid" name="eventid" />
                                ...
                                @foreach($Events as $events)
                                  <tr   id="row-{{$events->id}}" class="active">
                                    <td colspan="1">
                                      <input type="checkbox" class="checkrec"  id="rec-{{$events->id}}" value="{{$events->id}}" onclick="geteventid({{$events->id}})">
                                      <label for="rec-{{$events->id}}"></label>
                                    </td>
                                    <td>
                                        <img  src="{{asset('storage/' . $events->image )}}" width="29" height="35"  >
...
//     $('input.checkrec').on('change', function() {
//     $('input.checkrec').not(this).prop('checked', false);
// });
// });
...
function geteventid(id){
    ...
    var checkBox = document.getElementById("rec-"+id);
//    if (checkBox.checked == true){
    // $(".checkrec").attr("checked", false); //uncheck all checkboxes
//    $(checkBox).attr("checked", true);  //check the clicked one
//    $('input#rec-'+id).not('input#rec-'+id).prop('checked', false);
$('input[type="checkbox"]').not("rec-"+id).prop("checked", false);
   $(checkBox).prop("checked", true);  //check the clicked one
