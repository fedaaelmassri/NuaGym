    ...
    var url='{{URL::to('/admin/programs')}}';
         ...
         $.ajax({
            ...
            '  <td  class=" text-center" style="color:#000000 !important;">'+
                     ...
                     '<a      style="display:block"  href="#" id="'+pid+obj.id+'"  onclick="addcoache('+obj.id+')"> <i class="fas fa-check-circle text-success"></i></a>'+
           ...
           '<a href="#" style="display:none"    id="removecoach-'+pid+obj.id+'" onclick="deletcoach('+obj.id+')"  > <i class="fas fa-times-circle text-danger"></i></a>'+
                ...
                if( value.id==pid){
                    console.log('cochid',obj.id,'proid',value.id,'name',value.name, 'pid',pid,'rowid',"#addcoach-"+pid+obj.id);
                    }
                if( value.id==pid){
                $("#"+pid+obj.id).css("display", "none");
               $("#removecoach-"+pid+obj.id).css("display", "block");
              }
             if(value.id!=pid){
               $("#"+pid+obj.id).css("display", "block");
