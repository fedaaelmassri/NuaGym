    ...
    var url='{{URL::to('/admin/programs')}}';
         ...
         $.ajax({
            ...
            '  <td  class=" text-center" style="color:#000000 !important;">'+
                     ...
                     '<a   style="display:block"   href="#" id="addcoach-'+obj.id+'"  onclick="addcoache('+obj.id+')"> <i class="fas fa-check-circle text-success"></i></a>'+

           '<a href="#" class="" style="display:none"   id="removecoach-'+obj.id+'" onclick="deletcoach('+obj.id+')"  > <i class="fas fa-times-circle text-danger"></i></a>'+
                '</td></tr>');
                $.each(obj.programs, function(key, value) {
                    //  console.log(pid, '   ' ,value.id,'   ', $.inArray(pid, value.id));
                if( value.id==pid){
                    console.log('id',value.id,'name',value.name, 'pid',pid,'rowid',"#addcoach-"+obj.id);
                    }
                if( value.id==pid){
                $("#addcoach-"+obj.id).css("display", "none");
               $("#removecoach-"+obj.id).css("display", "block");
              }
             if(value.id!=pid){
               $("#addcoach-"+obj.id).css("display", "block");
               $("#removecoach-"+obj.id).css("display", "none");
            }
      });
      });
