...
class CreateCoacheClassesTable extends Migration
{
    ...
    {
        Schema::table('coache_classes', function (Blueprint $table) {
            $table->dropForeign('coache_classes_coache_id_foreign');
            $table->dropColumn('coache_id');
            $table->dropForeign('coache_classes_event_id_foreign');
